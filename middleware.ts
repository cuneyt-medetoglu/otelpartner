...
export async function middleware(req: NextRequest) {
  ...
  if (path.startsWith("/dashboard/catalog") && token?.role !== "guide" && token?.role !== "admin" && token?.role !== "hotel") return NextResponse.redirect(new URL("/dashboard", req.url));
  if (path.startsWith("/api/catalog") && token?.role !== "guide" && token?.role !== "admin" && token?.role !== "hotel") return NextResponse.json({ error: "Forbidden" }, { status: 403 });
